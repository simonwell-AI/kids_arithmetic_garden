...
- **答錯顯示正確答案**：FeedbackToast 答錯時顯示正確答案；今日任務／練習題依「自動朗讀」是否朗讀；速度測驗（綜合、加減乘除、乘法）答錯時亦顯示正確答案（不自動朗讀）  
...
- **蜜蜂蟲害**：花園新增第二種蟲害「蜜蜂」，進入花園時有機率觸發（與蟲害獨立）；顯示 3 隻蜜蜂 GIF（`bee_flying_animation.gif`），成長速率 ×0.6；「🐝 驅蜂」消耗 1 殺蟲劑，驅蜂計入除蟲 5 次成就；`GardenRecord` 新增 `hasBees`、`lastBeesRemovedAt`  
- **開花收成消耗園藝工具**：開花收成時自動消耗肥料瓶、噴霧器、盆栽土各 1（若有），下一株需再購買；`inventory.useTool(toolId)` 供扣減；園藝叉、園藝剪刀、園藝鏟不消耗；商店園藝工具區加註「肥料瓶、噴霧器、盆栽土在開花收成後會消耗，需再購買新的。」  
...
- **答錯回饋文案**：標題由「再試一次」改為「答錯了，沒關係～」；答案行改為「答案是 ○○，下一題加油～」；語音朗讀改為「答案是 ○○，下一題加油」  
- **今日任務答錯朗讀**：答錯時是否朗讀正確答案改為依「自動朗讀」勾選（`speakCorrectAnswer={speechEnabled}`）  
