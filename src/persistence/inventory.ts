...
function normalizeInv(record: InventoryRecord): InventoryRecord {
  return {
    ...
    insectFood: record.insectFood ?? 0,
...
export async function addInsectFood(count: number): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
  inv.insectFood = Math.max(0, (inv.insectFood ?? 0) + count);
  await (await getDB()).put(STORE_INVENTORY, inv);
}

export async function useInsectFood(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.insectFood ?? 0;
  if (n < 1) return false;
  inv.insectFood = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
...
export async function addMiteSpray(count: number): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
  inv.miteSpray = Math.max(0, (inv.miteSpray ?? 0) + count);
  await (await getDB()).put(STORE_INVENTORY, inv);
}

export async function useMiteSpray(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.miteSpray ?? 0;
  if (n < 1) return false;
  inv.miteSpray = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
...
export async function addStagBeetleLarva(count: number): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
  inv.stagBeetleLarva = Math.max(0, (inv.stagBeetleLarva ?? 0) + count);
  await (await getDB()).put(STORE_INVENTORY, inv);
}

export async function useStagBeetleLarva(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.stagBeetleLarva ?? 0;
  if (n < 1) return false;
  inv.stagBeetleLarva = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
}

export async function setHasInsectHabitat(has: boolean): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
  inv.hasInsectHabitat = has;
  await (await getDB()).put(STORE_INVENTORY, inv);
}

export async function addButterflyEgg(count: number): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
...
export async function useButterflyEgg(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.butterflyEgg ?? 0;
  if (n < 1) return false;
  inv.butterflyEgg = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
}
...
export async function addCicadaEgg(count: number): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = await getInventory();
...
export async function useCicadaEgg(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.cicadaEgg ?? 0;
  if (n < 1) return false;
  inv.cicadaEgg = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
...
export async function useAdvancedInsectGrowthMedicine(): Promise<boolean> {
  if (typeof window === "undefined") return false;
  const inv = await getInventory();
  const n = inv.advancedInsectGrowthMedicine ?? 0;
  if (n < 1) return false;
  inv.advancedInsectGrowthMedicine = n - 1;
  await (await getDB()).put(STORE_INVENTORY, inv);
  return true;
}
...
export async function removeInsectFood(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.insectFood = Math.max(0, (inv.insectFood ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
...
export async function removeMiteSpray(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.miteSpray = Math.max(0, (inv.miteSpray ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
...
export async function removeStagBeetleLarva(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.stagBeetleLarva = Math.max(0, (inv.stagBeetleLarva ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
...
export async function removeButterflyEgg(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.butterflyEgg = Math.max(0, (inv.butterflyEgg ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
...
export async function removeCicadaEgg(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.cicadaEgg = Math.max(0, (inv.cicadaEgg ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
...
export async function removeAdvancedInsectGrowthMedicine(count: number = 1): Promise<void> {
  if (typeof window === "undefined") return;
  const inv = normalizeInv(await getInventory());
  inv.advancedInsectGrowthMedicine = Math.max(0, (inv.advancedInsectGrowthMedicine ?? 0) - count);
  await (await getDB()).put(STORE_INVENTORY, inv);
